...
namespace MyCocktail.Infrastucture.Repositories
{
    public class CocktailDbSourceRepository
    {
        ...
        public async Task<IEnumerable<DrinkSourceDto>> GetCocktailSourcesByLetter(char letter)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var cockatails = JsonConvert.DeserializeObject<DrinksSourceDto>(responseBody);
                return cockatails.drinks;
            }
            System.Console.WriteLine("Error calling APi");
            return new List<DrinkSourceDto>();
        }
    }
}
